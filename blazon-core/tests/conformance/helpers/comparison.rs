...
impl ComparisonResult {
    ...
    pub fn write_comparison_files(&self, fixture_name: &str) {
        ...
        let blazon_path = full_dir.join(format!("{}.blazon.rs", filename));
        fs::write(&blazon_path, &self.blazon).unwrap();
        eprintln!("Wrote: {}", blazon_path.display());
    ...
    fn show_diff(&self) {
        // Reversed: rustfmt -> blazon (shows what blazon is doing wrong)
        let input = InternedInput::new(&self.rustfmt[..], &self.blazon[..]);
    ...
    fn generate_diff_content(&self) -> String {
        ...
        output.push_str(&format!("Blazon size:   {} bytes\n", self.blazon.len()));
        ...
        if diff.count_additions() == 0 && diff.count_removals() == 0 {
            ...
            output.push_str("=== DIFF (- rustfmt, + blazon) ===\n");
