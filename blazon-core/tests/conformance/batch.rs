...
fn compare_rust_analyzer_crate(crate_name: &str) {
        ...
        .filter(|e| e.path().extension().is_some_and(|ext| ext == "rs"))
    {
        ...
        total_bytes_blazon += result.blazon.len();
        total_bytes_rustfmt += result.rustfmt.len();

        let is_identical = result.blazon == result.rustfmt;
    ...
    eprintln!("Total size (blazon):   {} bytes", total_bytes_blazon);
